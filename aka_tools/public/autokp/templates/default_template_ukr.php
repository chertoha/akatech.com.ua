...
<div id=inner_pdf>  
    <div id=header_pdf>
		...
		03027, с.Новосілки,Києво-Святошинський р-н, вул. Садова 26, тел.: (067)463 35 23, info@akatech.com.ua, <a href="http://www.akatech.com.ua">www.akatech.com.ua</a>
		</div>
        <hr>
    </div> 
    ...
    <div id=greeting_pdf>
        ...
        <div class="commercial">
            <h3>ТЕХНІКО-КОМЕРЦІЙНА ПРОПОЗИЦІЯ</h3>
        </div>
    </div> 
    <div id="prod_pdf">
        <hr>
        ...
        function getTab($article) {
            ...
            if ($article['name']['on']) {
                $tab['Опис'] = $article['name']['val'] . ' ';
            }
            if ($article['description']['on']) {
                $tab['Опис'] = '' . $article['description']['val'];
            }
            ...
            $tab['К-ть'] = $article['qty'];
            $tab['Ціна, грн з ПДВ'] = $article['price'];
            $tab['Сума, грн з ПДВ'] = $article['sum'];
            if ($article['delivery_time']['on']) {
                $tab['Термін постачання'] = $article['delivery_time']['val'] . ' ';
            }
            return $tab;
        }
        ...
        <div class="total_sum">
            <h3> <b>Всього: <?=$data['common_data']['total_sum']?> грн з ПДВ</b></h3>
        </div>
